...
public static class AppConstants
{
    ...
    public static List<ServiceProvider> CreateDefaultServiceProviders() => new()
    {
        ...
        new("Доставка", "Иваново",      "Рахими Мир Вайс ИП",             "372800081397", "+79036320814"),
        new("Сборка",   "Ярославль",    "ИП Страхов Дмитрий Михайлович", "463224431946", "+79992601826", "vat5"),
        new("Доставка", "Ярославль",    "ИП Страхов Дмитрий Михайлович", "463224431946", "+79992601827", "vat5"),
        new("Сборка",   "Череповец",    "ИП Страхов Дмитрий Михайлович", "463224431946", "+79992601828", "vat5"),
        new("Доставка", "Череповец",    "ИП Страхов Дмитрий Михайлович", "463224431946", "+79992601829", "vat5"),
        new("Сборка",   "Архангельск",  "ИП Страхов Дмитрий Михайлович", "463224431946", "+79992601830", "vat5"),
        new("Доставка", "Архангельск",  "Луцевич И.А. ИП",                "290129283076", "+79643007800", "none", "another"),
        new("Сборка",   "Розница МИКС", "ИП Страхов Дмитрий Михайлович", "463224431946", "+79992601830", "vat5"),
        new("Доставка", "Розница МИКС", "ИП Страхов Дмитрий Михайлович", "463224431946", "+79992601831", "vat5"),
        new("Сборка",   "Котлас",       "Анкудинов И.Н.",                 "290402783932", "+79212929739"),
        new("Доставка", "Котлас",       "Котов О.В. ИП",                  "290401152613", "+79523066444"),
    };
    ...
    /// <summary>
    /// Возвращает тип НДС агента по городу для 1С-реализаций (когда тип услуги неизвестен).
    /// vat5 только если ВСЕ агенты данного города — Страхов (ИНН 463224431946).
    /// Если в городе несколько разных агентов (Иваново, Архангельск) — возвращает "none" (безопасный дефолт).
    /// </summary>
    public static string GetServiceVatTypeByCity(string city)
    {
        if (string.IsNullOrWhiteSpace(city)) return "none";
        var norm = city.Trim();
        var matches = ServiceProviders.Where(p =>
            norm.Contains(p.City, StringComparison.OrdinalIgnoreCase) ||
            p.City.Contains(norm, StringComparison.OrdinalIgnoreCase)).ToList();
        if (matches.Count == 0) return "none";
        // vat5 только если все совпавшие агенты — Страхов
        return matches.All(p => p.Inn == "463224431946") ? "vat5" : "none";
    }
