...
public enum CorrectionScenario
{
    ...
    /// «Чек большей суммой».
    /// → Обычная отмена ошибочной суммы с тегом 1192 + правильный чек коррекции.
    /// </summary>
    CheckLargerAmount,
    ...
    /// → Обычная отмена с ФП исходного чека в теге 1192 + чек коррекции.
    /// </summary>
    CheckSmallerAmount,

    /// <summary>
    /// Чек вообще не был пробит — есть документ в 1С, но фискального чека нет.
    /// «Чек не пробит», «не вышел», «чека не было».
    /// → Только sell_correction (refund не нужен — чека и так нет).
    /// «Не день в день» сюда не относится: чек уже существует, см. WrongDate.
    /// </summary>
    CheckNotPunched,
    ...
    /// → Обычная отмена с тегом 1192 + чек коррекции с верным payments.type.
    /// </summary>
    WrongPaymentType,
    ...
    /// → Обычная отмена с тегом 1192 + чек коррекции.
    /// </summary>
    WrongNomenclature,
    ...
    /// <summary>
    /// Чек пробит не в день документа.
    /// Пример: реализация 20.10, чек 22.10.
    /// «Не день в день», «чек другой датой», «на следующий день».
    /// → Обычный обратный чек с ФП в теге 1192, затем чек коррекции;
    ///    в correction_info.base_date — дата исходного документа.
    /// </summary>
    WrongDate,
...
public enum OrderKind
{
    ...
    /// <summary>Пара документов: отмена исходного + правильный чек коррекции</summary>
